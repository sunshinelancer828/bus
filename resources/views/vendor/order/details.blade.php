...
@section('content')
                                        ...
                                        <div class="table-responsive-sm">
                                            <table class="table">
                                                ...
                                                @php 
                                                    ...
                                                    $price = $order->vendororders()->where('user_id','=',$user->id)->sum('total_price');
 ...
 <tr>
                                                    ...
                                                    <td width="10%">:</td>
                                                    <td width="45%">{{$order->account_name}}</td>
                                                </tr>  
                                             
 @if( $order->screenshot !='' )												
                                                 <tr>
                                                    <th width="45%">{{ __('Attached Screenshot') }}</th>
                                                    <td width="10%">:</td>
                                                    <td width="45%"> <a href="{{asset('assets/order_screenshot/'.$order->screenshot)}}" target="_blank" download />
                  <img class="xzoom-gallery5" width="80" src="{{asset('assets/order_screenshot/'.$order->screenshot)}}" title="">
                    </a></td>
                                                </tr>	
												       @endif
												            @endif
                ...
                <td width="45%">{{$order->shipping_zip == null ? $order->customer_zip : $order->shipping_zip}}</td>
                            ...
                            <div class="row">
                                ...
                                @foreach($cart->items as $key => $product)
                                    ...
                                    @php
                                    $newProduct = App\Models\Product::find($product['item']['id']);
                                    @endphp
                              ...
                              <a href="javascript:;" data-toggle="modal" data-target="#confirm-delete" class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i class="fa fa-eye"></i> View License</a>
                                            ...
                                            <td>
                                               ...
                                               @if($product['color'])
                                                ...
                                                @endif
                                                <p>
                                                        <strong>{{ $langg->lang754 }} :</strong> {{$order->currency_sign}}{{ round($newProduct->price * $order->currency_value , 2) }}
                                                </p>
                                               <p>
                                            ...
                                            <td>{{$order->currency_sign}}{{ round($newProduct->price * $order->currency_value , 2) }}</td>
