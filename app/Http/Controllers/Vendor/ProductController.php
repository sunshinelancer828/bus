    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Product $data) {
                                ...
                                if($data->status == 0 && $data->rejected !=  '' ) {
                                    return '<div class="action-list"><select class="process select droplinks '.$class.'"><<option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Rejected</option>/select></div>';
                                } else if ($data->status == 0 && $data->rejected ==  '' ) {
                                    return '<div class="action-list"><select class="process select droplinks '.$class.'"><<option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Admin Approval Pending</option>/select></div>';
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
       ...
       // if($prods < $package->allowed_products || $package->allowed_products == 0){
           //custom code added so that user can upload products without having any subscription
         $allow_free = 'yes';
        if($allow_free == 'yes') {
        //--- Validation Section
        $rules = [
        ...
        //--- Logic Section
            ...
            {      
                ...
                $tmp_file = tempnam(public_path('assets/images/products'), $name);
                copy($input['default_photo'], $tmp_file);
                return response()->json(array('errors' => [ 0 => 'You got a default photo: '. $name . $tmp_file->getSize()]));
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (empty($request->tags))
		 ...
		 $check_rejected = DB::table('products')->where([
        ['id', '=', $id],
        ['rejected', '!=', ''],
		])->first();
		if( $check_rejected != '' || $check_rejected != null ) {
